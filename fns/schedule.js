...
module.exports = (d) => {
  ...
  for (
    ...
    v.пункт_отчета.дата = f.формат_даты(v.дата_расчета)
    ...
    v.пункт_отчета.осн_долг = f.округлить(v.остаток_осн_долга)
    ...
    v.пункт_отчета.проценты = f.округлить(v.процентов_с_даты_платежа)

    // считаем долг с процентами на дату расчета
    v.долг_с_процентами = v.остаток_осн_долга + v.процентов_с_даты_платежа
    v.пункт_отчета.долг_с_процентами = f.округлить(v.долг_с_процентами)
    ...
    if (v.списано_средств > 0) {
      ...
      v.пункт_отчета.выплачено_всего = f.округлить(v.выплачено_всего)

      v.процентов_после_списания = v.процентов_с_даты_платежа - v.списано_средств

      v.на_проценты =
        v.процентов_после_списания <= 0 ? v.процентов_с_даты_платежа : v.процентов_после_списания
      v.пункт_отчета.на_проценты = f.округлить(v.на_проценты)
      v.на_осн_долг =
        v.процентов_после_списания < 0 ? Math.abs(v.процентов_после_списания) : 0
      v.пункт_отчета.на_осн_долг = f.округлить(v.на_осн_долг)

      v.процентов_с_даты_платежа = v.процентов_с_даты_платежа - v.на_проценты
      v.остаток_осн_долга = v.остаток_осн_долга - f.округлить(v.на_осн_долг)

      v.пункт_отчета.остаток_осн_долга = f.округлить(v.остаток_осн_долга)

      v.выплачено_процентов = v.выплачено_процентов + v.на_проценты
      v.пункт_отчета.выплачено_процентов = f.округлить(v.выплачено_процентов)
    }
